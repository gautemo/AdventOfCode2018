...
const startSky = () => {
    canvas.height = 1000;
    canvas.width = 1000;
    interval = setInterval(runSecond, 1);
}

const runSecond = () => {
    const diff = Math.abs((lights[0].x - lights[1].x));
    if(interval && diff < 50){ //If lights are getting close; stop interval and run seconds manually
...
const parseInput = () => {
    ...
    for(let l of lines){
        ...
        lights.push(new Light(x, y, parseInt(p[2]), parseInt(p[3])));
    }
}
